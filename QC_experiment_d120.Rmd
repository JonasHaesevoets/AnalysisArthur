...
file_names_tibble <- tribble(
    ...
    "d120",
    #library_name
    paste0(file_path$intermediate_data,"seurat_obj_d120_raw_dbec.rds"),
    #raw_seurat_obj_path
    paste0(file_path$raw_data_root,"output_J120/BD-Analysis-BMachiels-EXP3_Sample_Tag_Calls.csv"),
    #path_sample_tag_calls
    paste0(file_path$intermediate_data,"seurat_obj_d120_workflowed.rds"),
    #write_processed_seurat_obj_path
)
```
...
seurat_sampletag_name <- function(seurat_obj, sample_tag_calls){
    ...
    seurat_obj@meta.data$sampletag_name = factor(seurat_obj@meta.data$sampletag_name,     levels = c("Multiplet", "Undetermined", "Mock_r1", "Mock_r2","MuHV4_r1", "MuHV4_r2", "MuHV4_r3","del73_r1", "del73_r2", "del73_r3"))
...
```{r}
seurat_d120_raw = read_rds(file_names_tibble$raw_seurat_obj_path)
...
seurat_d120_raw = seurat_sampletag_name(seurat_obj = seurat_d120_raw, sample_tag_calls = sample_tag_calls)
seurat_d120_raw$amountUMI = seurat_d120_raw$nCount_RNA
seurat_d120_raw$amountGenes = seurat_d120_raw$nFeature_RNA

```

# 2) QC and it's associated metrics

the object at day 120 contains 35808 different cells and 28958 genes.
...
```{r}
seurat_d120_raw$log10GenesPerUMI = log10(seurat_d120_raw$nFeature_RNA) / log10(seurat_d120_raw$nCount_RNA)
```

## 2.2 Calculate the mitochondrial rate

```{r}
seurat_d120_raw$mitoRatio <- PercentageFeatureSet(object = seurat_d120_raw, pattern = "^mt-")
seurat_d120_raw$mitoRatio <- seurat_d120_raw@meta.data$mitoRatio / 100
```
...
```{r, echo=FALSE}
seurat_d120_raw <- dplyr::filter(seurat_d120_raw, sampletag_multiplets == "single_hashtag" & !is.na(sampletag_name))
...
```{r}
seurat_d120_filtered = dplyr::filter(seurat_d120_raw, 
                                (mitoRatio < 0.2) &
                                (amountGenes > 500) &
                                (amountUMI > 5000) &
...
```{r}
countsDataset <- GetAssayData(object = seurat_d120_filtered, slot = "counts")
...
seurat_d120_filtered1 <- CreateSeuratObject(counts = filtered_counts, meta.data = seurat_d120_filtered@meta.data)
# copy them back from the object where the genes werent filtered
seurat_d120_filtered1@assays$protein = seurat_d120_filtered@assays$protein
seurat_d120_filtered1@assays$sampletags = seurat_d120_filtered@assays$sampletags
```

# After QC we retain 22846 genes and 16467 cells from 28958 genes and 21145 cells
...
```{r}
seurat_d120_filtered_normalized = NormalizeData(seurat_d120_filtered1)
```
...
seurat_d120_filtered_normalized <- CellCycleScoring(seurat_d120_filtered_normalized,
...
#| fig-width: 10
#| fig-height: 8
VlnPlot(seurat_d120_filtered_normalized, features = c("S.Score", "G2M.Score"), group.by = "sampletag_name")
```
...
```{r}
seurat_d120_filtered_normalized <- FindVariableFeatures(seurat_d120_filtered_normalized, 
                     selection.method = "vst",
                     nfeatures = 2000, 
                     verbose = FALSE)
seurat_d120_filtered_normalized = ScaleData(seurat_d120_filtered_normalized)
seurat_d120_filtered_normalized = RunPCA(seurat_d120_filtered_normalized)
...
#| fig-width: 10
#| fig-height: 8
DimPlot(seurat_d120_filtered_normalized,
        reduction = "pca",
        group.by= "Phase",
        split.by = "Phase")
```
...
```{r}
seurat_d120_filtered_normalized |> write_rds("intermediate_data/seurat_obj_d120_afterQCdashboard.rds")
```
