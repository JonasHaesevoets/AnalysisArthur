...
file_names_tibble <- tribble(
    ...
    "d38",
    #library_name
    paste0(file_path$intermediate_data,"seurat_obj_d38_raw_dbec.rds"),
    #raw_seurat_obj_path
    paste0(file_path$raw_data_root,"output_J38/BD-Analysis-BMachiels-J38_Sample_Tag_Calls.csv"),
    #path_sample_tag_calls
    paste0(file_path$intermediate_data,"seurat_obj_d38_workflowed.rds"),
    #write_processed_seurat_obj_path
)
```
...
seurat_sampletag_name <- function(seurat_obj, sample_tag_calls){
    ...
    seurat_obj@meta.data$sampletag_name = factor(seurat_obj@meta.data$sampletag_name,     levels = c("Multiplet", "U ndetermined", "Mock_r1", "Mock_r2","Mock_r3", "MuHV4_r1", "MuHV4_r2", "MuHV4_r3","del73_r1", "del73_r2", "del73_r3"))
...
```{r}
seurat_d38_raw = read_rds(file_names_tibble$raw_seurat_obj_path)
...
seurat_d38_raw = seurat_sampletag_name(seurat_obj = seurat_d38_raw, sample_tag_calls = sample_tag_calls)
seurat_d38_raw$amountUMI = seurat_d38_raw$nCount_RNA
seurat_d38_raw$amountGenes = seurat_d38_raw$nFeature_RNA
...
```{r}
seurat_d38_raw$log10GenesPerUMI = log10(seurat_d38_raw$nFeature_RNA) / log10(seurat_d38_raw$nCount_RNA)
```

## 2.2 Calculate the mitochondrial rate

```{r}
seurat_d38_raw$mitoRatio <- PercentageFeatureSet(object = seurat_d38_raw, pattern = "^mt-")
seurat_d38_raw$mitoRatio <- seurat_d38_raw@meta.data$mitoRatio / 100
```
...
```{r, echo=FALSE}
seurat_d38_raw = dplyr::filter(seurat_d38_raw, sampletag_multiplets == "single_hashtag")

```

### after filtering for singlets only 24912 cells out of 29234 were retained
...
### less cells for Mock second replicate compared to the rest
...
```{r}
seurat_d38_filtered = dplyr::filter(seurat_d38_raw, 
                                (mitoRatio < 0.2) &
                                (amountGenes > 500) &
                                (amountUMI > 5000) &
...
20208 cells out of 25962 cells were retained
...
```{r}
countsDataset <- GetAssayData(object = seurat_d38_filtered, slot = "counts")
...
seurat_d38_filtered1 <- CreateSeuratObject(counts = filtered_counts, meta.data = seurat_d38_filtered@meta.data)
# copy them back from the object where the genes werent filtered
seurat_d38_filtered1@assays$protein = seurat_d38_filtered@assays$protein
seurat_d38_filtered1@assays$sampletags = seurat_d38_filtered@assays$sampletags
```

# After QC we retain 236632 genes and 20208 cells from 28921 genes and 23663 cells
...
```{r}
seurat_d38_filtered_normalized = NormalizeData(seurat_d38_filtered1)
```
...
seurat_d38_filtered_normalized <- CellCycleScoring(seurat_d38_filtered_normalized,
...
#| fig-width: 10
#| fig-height: 8
VlnPlot(seurat_d38_filtered_normalized, features = c("S.Score", "G2M.Score"), group.by = "sampletag_name")
```
...
```{r}
seurat_d38_filtered_normalized <- FindVariableFeatures(seurat_d38_filtered_normalized, 
                     selection.method = "vst",
                     nfeatures = 2000, 
                     verbose = FALSE)
seurat_d38_filtered_normalized = ScaleData(seurat_d38_filtered_normalized)
seurat_d38_filtered_normalized = RunPCA(seurat_d38_filtered_normalized)
...
#| fig-width: 10
#| fig-height: 8
DimPlot(seurat_d38_filtered_normalized,
        reduction = "pca",
        group.by= "Phase",
        split.by = "Phase")
```
...
```{r}
# seurat_d38_filtered_normalized |> write_rds("intermediate_data/seurat_obj_d38_afterQCdashboard.rds")
```
